...
export function getWeatherSignalSignature({ date, signal }) {
  return `${date}:${[...(signal?.reasons || [])].sort().join("+") || "none"}`
}

export function evaluateWeatherLiveBoundary({
  shadowEligible,
  sendEnabled,
  quietHours,
  cooldownActive,
  ...
  if (!shadowEligible) return { allowed: false, reason: "shadow_not_eligible" }
  if (!sendEnabled) return { allowed: false, reason: "send_disabled" }
  if (quietHours) return { allowed: false, reason: "quiet_hours" }
  if (cooldownActive) return { allowed: false, reason: "proactive_cooldown" }
  if (dailyLimitReached) return { allowed: false, reason: "daily_proactive_limit" }
  if (userCurrentlyActive) return { allowed: false, reason: "user_currently_active" }
  if (alreadySent) return { allowed: false, reason: "weather_signal_already_sent" }
  return { allowed: true, reason: "limited_send_eligible" }
}

export function parseWeatherMessageDecision(raw) {
  try {
    const source = String(raw || "").trim().replace(/^```json\s*/i, "").replace(/```$/i, "").trim()
    const data = JSON.parse(source)
    return {
      ...
      motivation: String(data.contact_motivation || "").trim().slice(0, 120),
      message: String(data.message || "").trim().replace(/\s+/g, " ").slice(0, 90),
    }
  } catch {
    return { parsed: false, shouldSend: false, motivation: "", message: "" }
  }
}
