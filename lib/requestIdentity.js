...
function identityError(code, status = 401, metadata = {}) {
...
export function configuredPrivateAuthUserUuid(env = process.env) {
  const value = String(env.XIAOC_PRIVATE_AUTH_USER_UUID || "").trim().toLowerCase()
  if (!value) throw identityError("private_owner_not_configured", 503)
  if (!UUID_PATTERN.test(value)) throw identityError("private_owner_config_invalid", 503)
  return value
}

function authProviderFailureClass(error) {
  ...
  if (["ECONNRESET", "ECONNREFUSED", "ENETUNREACH", "EHOSTUNREACH", "EAI_AGAIN"].includes(code)) {
    return "network"
  }
...
function authVerificationError(error) {
  const providerFailureClass = authProviderFailureClass(error)
  if (providerFailureClass) {
    return identityError("auth_provider_unavailable", 503, {
      retryable: true,
      providerFailureClass,
    })
  }
  return identityError("invalid_access_token", 401, { retryable: false })
}
...
async function loadActiveCompanion(serviceClient, authUserId) {
  ...
  if (data.user_id !== authUserId) throw identityError("companion_owner_mismatch", 403)
  if (data.lifecycle_status !== "active") throw identityError("companion_inactive", 403)
  return data
}
...
export async function resolveRequestIdentity(req, {
  ...
  if (token) {
    ...
    if (error) throw authVerificationError(error)
    if (!data?.user) throw identityError("invalid_access_token", 401, { retryable: false })
    ...
    const expectedOwnerUuid = configuredPrivateAuthUserUuid(env)
    if (claims.sub !== expectedOwnerUuid) throw identityError("private_account_mismatch", 403)
    ...
    const identity = {
      ...
      companionStatus: companion.lifecycle_status,
    }
    shadowLog({ route, result: "pass", identitySource: identity.identitySource })
    return identity
  }
  ...
  if (String(env.PRIVATE_IDENTITY_APP_TOKEN_FALLBACK_ENABLED || "") === "true") {
    ...
    if (!supplied) throw identityError("authentication_required")
    ...
    const expectedOwnerUuid = configuredPrivateAuthUserUuid(env)
    const companion = await loadActiveCompanion(companionClient, expectedOwnerUuid)
    assertCompatibleClientOwner(req)
    const identity = {
      actorType: "authenticated_user",
...
export async function requireRequestIdentity(req, res, options) {
  try {
    ...
    if (error?.providerFailureClass) body.providerFailureClass = error.providerFailureClass
    res.status(error?.status || 401).json(body)
    return null
  }
}
