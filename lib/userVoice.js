...
export const USER_VOICE_MAX_DURATION_SECONDS = 120
...
export async function transcribeAndStoreUserVoice({
  ...
  const providerDuration = Number.isFinite(Number(data?.duration)) ? Number(data.duration) : duration
  const segments = Array.isArray(data?.segments) ? data.segments : []
  const averageNoSpeechProbability = segments.length
    ? segments.reduce((total, segment) => total + (Number(segment?.no_speech_prob) || 0), 0) / segments.length
    : 0
  if (!transcript || providerDuration < 0.6 || averageNoSpeechProbability >= 0.8) {
