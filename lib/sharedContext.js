...
export const SHARED_CONTEXT_PARSE_FAILURE_BACKOFF_MS = 30 * 60 * 1000
...
export function getSharedContextDiagnostics(value, { injected = false, update = null } = {}) {
  ...
  return {
    ...
    update_failed_at: update?.failedAt || null,
...
export function selectPendingSharedContextMessages(messages, workingContext, conversationId = null) {
  ...
  return checkpointIndex >= 0 ? eligible.slice(checkpointIndex + 1) : []
}

export function getSharedContextParseFailureBackoff(
  messages,
  sharedContextId,
  now = new Date(),
) {
  ...
  const failure = [...(messages || [])].reverse().find(message => (
    ...
    && message?.metadata?.sharedContext?.id === sharedContextId
    && message?.metadata?.sharedContext?.update_reason === "parse_failed"
    && message?.metadata?.sharedContext?.update_failed_at
  ))
  const failureMs = new Date(failure?.metadata?.sharedContext?.update_failed_at || "").getTime()
  if (!Number.isFinite(nowMs) || !Number.isFinite(failureMs)) return null
  const retryAtMs = failureMs + SHARED_CONTEXT_PARSE_FAILURE_BACKOFF_MS
  return retryAtMs > nowMs
    ? { reason: "parse_failure_backoff", retryAt: new Date(retryAtMs).toISOString() }
    : null
}
