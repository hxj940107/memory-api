...
// MiniMax China pay-as-you-go pricing, verified 2026-09-04:
// https://platform.minimaxi.com/docs/guides/pricing-paygo
const MINIMAX_SPEECH_PRICE_CNY_PER_10K = {
  "speech-2.8-hd": 3.5,
  "speech-2.6-hd": 3.5,
  "speech-02-hd": 3.5,
  "speech-01-hd": 3.5,
...
export function getMiniMaxSpeechPriceCnyPer10k(model) {
  return MINIMAX_SPEECH_PRICE_CNY_PER_10K[String(model || "").trim()] ?? null
}

export function estimateMiniMaxSpeechCostCny({ model, usageCharacters }) {
  const price = getMiniMaxSpeechPriceCnyPer10k(model)
  const characters = Number(usageCharacters)
  if (price === null || !Number.isFinite(characters) || characters < 0) return null
  return characters * price / 10_000
}
...
export function getMiniMaxSpeechConfig(env = process.env) {
  ...
  const secondaryVoiceId = String(env.MINIMAX_SECONDARY_VOICE_ID || "").trim()
  const primaryVoiceWeight = Math.round(boundedNumber(env.MINIMAX_PRIMARY_VOICE_WEIGHT, 75, 1, 100))
  const secondaryVoiceWeight = Math.round(boundedNumber(env.MINIMAX_SECONDARY_VOICE_WEIGHT, 25, 1, 100))
  const timbreWeights = secondaryVoiceId
    ? [
        { voice_id: voiceId, weight: primaryVoiceWeight },
        { voice_id: secondaryVoiceId, weight: secondaryVoiceWeight },
      ]
    : null
  ...
  return {
    ...
    async synthesize({ text }) {
      ...
      try {
        response = await fetchImpl(`${config.baseUrl}/v1/t2a_v2`, {
          ...
          body: JSON.stringify({
            ...
            ...(config.timbreWeights ? { timbre_weights: config.timbreWeights } : {}),
            audio_setting: {
              sample_rate: 32000,
              bitrate: 128000,
              format: "mp3",
      ...
      return {
        ...
        model: config.model,
        usage_characters: Number.isFinite(Number(payload?.extra_info?.usage_characters))
          ? Math.max(0, Number(payload.extra_info.usage_characters))
          : null,
        estimated_cost_cny: estimateMiniMaxSpeechCostCny({
          model: config.model,
          usageCharacters: payload?.extra_info?.usage_characters,
        }),
        price_cny_per_10k: getMiniMaxSpeechPriceCnyPer10k(config.model),
      }
    },
  }
}
