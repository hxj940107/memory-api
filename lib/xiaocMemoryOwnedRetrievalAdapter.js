...
function ownedRepository(repository, contentById, excludedMemoryIds = []) {
  const excluded = new Set((excludedMemoryIds || []).map(String).filter(Boolean))
  const allowed = row => {
    const memory = row?.memory || row
    if (excluded.has(String(memory?.id || memory?.memory_id || ""))) return false
...
export async function retrieveOwnedMemoryPromptCandidatesShadow({
  ...
  const retrieval = await retrieveXiaoCMemoriesOffline({
    repository: ownedRepository(source, contentById, excludedMemoryIds),
    userId,
    query,
    retrievalTime,
    mode: "normal_current",
    explicitRecall: false,
    topK: XIAOC_MEMORY_RETRIEVAL_POLICY.maximumTopK,
    queryEmbedding,
  ...
  const promptReadyCandidates = gateway.injected
  ...
  const rankingByMemoryId = new Map(
    (retrieval.trace?.ranking_decisions || []).map(item => [String(item.memory_id), item])
  )
  ...
  return {
    ...
    telemetry: {
      ...
      discovered_candidate_count: retrieval.trace?.discovered_candidate_count || retrieval.trace?.raw_candidate_count || 0,
      eligibility_rejected_count: retrieval.trace?.eligibility_rejected_count || 0,
      eligibility_rejected_reasons: retrieval.trace?.reason_code_counts || {},
      ranked_pool_count: rankedPool.length,
      ...
      injected_candidates: promptReadyCandidates.map(item => {
        const ranked = rankingByMemoryId.get(String(item.memoryId))
        return {
          memory_id: item.memoryId,
          rank: ranked?.rank ?? null,
          signal_mode: ranked?.component_scores?.semantic === null
            ? "lexical_only"
            : ranked?.component_scores?.lexical > 0 ? "hybrid" : "semantic_only",
          lexical_score: ranked?.component_scores?.lexical ?? null,
          semantic_score: ranked?.component_scores?.semantic ?? null,
          relevance_score: ranked?.component_scores?.relevance ?? null,
          final_score: ranked?.hybrid_score ?? null,
        }
      }),
      trace: {
        candidates: (retrieval.trace?.ranking_decisions || []).map((item, index) => ({
          ...
          relevance_score: item.component_scores?.relevance ?? null,
