  ...
  return {
    async listLexicalCandidates(args) {
      ...
      return rows.filter(row => (
        row.origin_system === "xiaoc_native"
        || (
          row.origin_system === "ombre_legacy"
          && row.lifecycle_status === "active"
          && row.provenance_status === "legacy_unverified"
          && row.retrieval_tier === "low_authority"
          && row.authority_tier === "legacy_limited"
        )
      )).map(row => {
