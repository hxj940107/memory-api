...
import { retrieveOwnedMemoryPromptCandidatesShadow } from "./xiaocMemoryOwnedRetrievalAdapter.js"
import {
...
async function emitTelemetry({ client, env, userId, logger, level, telemetry }) {
  logTelemetry(logger, level, telemetry)
  await bestEffortWriteXiaoCMemoryObservationAudit({
    client,
    env,
    row: buildXiaoCMemoryRetrievalAuditRow({ env, userId, telemetry }),
    logger,
  })
  return telemetry
}

function decisionTelemetry({ correlationIdHash, eligibleOpportunity, sampled = false, skippedReason, errorCode = null }) {
  return {
...
export function normalizeXiaoCShadowResults(results) {
  return (results || []).map((item, index) => ({
    ...
    origin_system: item?.origin_system ?? "unknown",
...
export async function runXiaoCMemoryShadowRead({
  ...
  logger = console, repository = null, ownedOnly = false, promptContext = {}, memoryBudget = null,
} = {}) {
  const config = getXiaoCMemoryShadowConfig(env)
  if (!config.enabled) return { attempted: false, skipped_reason: "FLAG_OFF" }
  if (!config.sampleRate && !isXiaoCMemoryObservationAuditEnabled(env)) return { attempted: false, skipped_reason: "SAMPLE_RATE_ZERO" }
  const correlationIdHash = buildXiaoCMemoryCorrelationHash(correlationId)
  if (!trustedUserId || requestedUserId !== trustedUserId) {
    return emitTelemetry({ client, env, userId: requestedUserId || trustedUserId, logger, level: "warn", telemetry: decisionTelemetry({
      correlationIdHash, eligibleOpportunity: false, skippedReason: "TRUSTED_USER_MISSING", errorCode: "TRUSTED_USER_MISSING",
    }) })
  }
  if (LOW_SIGNAL_MESSAGE.test(String(message || "").trim())) {
    return emitTelemetry({ client, env, userId: trustedUserId, logger, level: "log", telemetry: decisionTelemetry({ correlationIdHash, eligibleOpportunity: false, skippedReason: "QUERY_PLAN_SKIP" }) })
  }
  ...
  try {
    ...
    if (!isXiaoCMemoryShadowSampled(correlationId, config.sampleRate)) {
      return emitTelemetry({ client, env, userId: trustedUserId, logger, level: "log", telemetry: decisionTelemetry({ correlationIdHash, eligibleOpportunity: true, skippedReason: "NOT_SAMPLED" }) })
    }
    ...
    const task = (async () => {
      const timedRepository = withTiming(dbRepository, latency)
      const owned = ownedOnly ? await retrieveOwnedMemoryPromptCandidatesShadow({
        repository: timedRepository, userId: trustedUserId, query: message,
        retrievalTime: now(), context: promptContext, memoryBudget,
      }) : null
      const result = owned?.retrieval || await retrieveXiaoCMemoriesOffline({
        repository: timedRepository, userId: trustedUserId, query: message,
      ...
      return { result, ownedPromptReadiness: owned?.telemetry || null }
    })()
    let timeoutId
    const remainingTimeoutMs = Math.max(0, config.timeoutMs - (performance.now() - started))
    const taskResult = await Promise.race([task, new Promise((_, reject) => {
    ...
    const telemetry = {
      ...
      correlation_id_hash: correlationIdHash, query_fingerprint: safeHash(queryPlan.retrieval_query),
      ...
      xiaoc: {
        ...
        origin_distribution: distribution(xiaoc, "origin_system"),
        reason_code_counts: result.trace?.reason_code_counts || {},
      },
      comparison: compareShadowResults(ombre, xiaoc),
      prompt_readiness: taskResult.ownedPromptReadiness,
      latency: { ...latency, shadow_total_ms: Math.round((performance.now() - started) * 1000) / 1000 },
      error_code: null,
    }
    return emitTelemetry({ client, env, userId: trustedUserId, logger, level: "log", telemetry })
  } catch (error) {
    const telemetry = {
      ...
      degradation_mode: queryPlan ? "LEXICAL_ONLY" : null, correlation_id_hash: correlationIdHash,
    ...
    return emitTelemetry({ client, env, userId: trustedUserId || requestedUserId, logger, level: "warn", telemetry })
  }
}
