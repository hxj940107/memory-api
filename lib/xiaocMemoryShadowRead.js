...
function sampled(correlationId, rate) {
...
function errorCode(error) {
  ...
  return /^[A-Z0-9_]{1,80}$/.test(value) ? value : "SHADOW_UNEXPECTED_ERROR"
}
...
export async function runXiaoCMemoryShadowRead({
  ...
  if (!trustedUserId || requestedUserId !== trustedUserId) return { attempted: false, skipped_reason: "TRUSTED_USER_SCOPE_MISSING" }
  if (!sampled(correlationId, config.sampleRate)) return { attempted: false, skipped_reason: "NOT_SAMPLED" }
  if (LOW_SIGNAL_MESSAGE.test(String(message || "").trim())) return { attempted: false, skipped_reason: "QUERY_PLAN_SKIP" }
  ...
  try {
    ...
    if (!queryPlan.should_retrieve) return { attempted: false, skipped_reason: "QUERY_PLAN_SKIP" }
    ...
    const result = await Promise.race([task, new Promise((_, reject) => {
      timeoutId = setTimeout(() => reject(new Error("SHADOW_TIMEOUT")), config.timeoutMs)
    })]).finally(() => clearTimeout(timeoutId))
    const ombre = normalizeOmbreShadowResults(ombreResults)
    const xiaoc = normalizeXiaoCShadowResults(result.results)
    const telemetry = {
      event: "xiaoc_memory_shadow_read", attempted: true, skipped_reason: null,
    ...
    logger.log("XIAOC MEMORY SHADOW:", telemetry)
    return telemetry
  } catch (error) {
    const telemetry = {
      event: "xiaoc_memory_shadow_read", attempted: true, skipped_reason: null,
    ...
    logger.warn("XIAOC MEMORY SHADOW:", telemetry)
    return telemetry
  }
}
