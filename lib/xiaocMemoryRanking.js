...
export const XIAOC_MEMORY_RETRIEVAL_POLICY = Object.freeze({
  ...
  semanticDistributionAdmission: Object.freeze({
    absoluteFloor: 0.5,
    finalScoreFloor: 0.5,
    separationMinimum: 0.1,
    maximumLeadingCluster: 2,
  }),
  hybridAdmission: Object.freeze({ lexicalMinimum: 0.2, semanticMinimum: 0.35, combinedMinimum: 0.27 }),
...
export function rankEligibleCandidate(candidate, context = {}, policy = XIAOC_MEMORY_RETRIEVAL_POLICY) {
  ...
  const normalThreshold = policy.thresholds[mode]
  const normalThresholdPassed = relevance >= normalThreshold
  const groundedHybrid = !normalThresholdPassed && mode === "hybrid"
    && candidate.lexical_score >= policy.hybridAdmission.lexicalMinimum
    && candidate.semantic_score >= policy.hybridAdmission.semanticMinimum
    && relevance >= policy.hybridAdmission.combinedMinimum
  const threshold = groundedHybrid ? policy.hybridAdmission.combinedMinimum : normalThreshold
  const passesThreshold = normalThresholdPassed || groundedHybrid
  const reasons = [
    ...
    ...(groundedHybrid ? ["GROUNDED_HYBRID_DUAL_SIGNAL"] : []),
...
function applySemanticDistributionAdmission(ranked, policy) {
  const config = policy.semanticDistributionAdmission
  if (!config) return ranked
  const semanticOnly = ranked.filter(item => item.signal_mode === "semantic_only")
  const maximumBoundary = Math.min(config.maximumLeadingCluster, semanticOnly.length - 1)
  let admittedCount = 0
  let strongestBoundary = -Infinity

  for (let size = 1; size <= maximumBoundary; size += 1) {
    const leading = semanticOnly.slice(0, size)
    if (!leading.every(item => (
      item.component_scores.semantic >= config.absoluteFloor
      && item.hybrid_score >= config.finalScoreFloor
    ))) continue
    const last = leading[leading.length - 1]
    const next = semanticOnly[size]
    const semanticGap = last.component_scores.semantic - next.component_scores.semantic
    const finalScoreGap = last.hybrid_score - next.hybrid_score
    const boundaryStrength = Math.min(semanticGap, finalScoreGap)
    if (boundaryStrength >= config.separationMinimum && boundaryStrength > strongestBoundary) {
      strongestBoundary = boundaryStrength
      admittedCount = size
    }
  }

  if (!admittedCount) return ranked
  const admittedIds = new Set(semanticOnly.slice(0, admittedCount).map(item => item.candidate.memory_id))
  return ranked.map(item => {
    if (item.passes_threshold || !admittedIds.has(item.candidate.memory_id)) return item
    return {
      ...item,
      passes_threshold: true,
      selection_reason_codes: [
        ...item.selection_reason_codes.filter(reason => reason !== "RELEVANCE_THRESHOLD_NOT_MET"),
        "SEMANTIC_DISTRIBUTION_SEPARATED",
...
export async function generateBoundedCandidates({ repository, userId, query, lexicalTerms = null, queryEmbedding = null, embeddingIdentity = null, channelMode = "hybrid", policy = XIAOC_MEMORY_RETRIEVAL_POLICY }) {
  ...
  const lexicalPromise = lexicalEnabled ? repository.listLexicalCandidates({ userId, query, lexicalTerms, limit: policy.lexicalCandidateLimit }) : Promise.resolve([])
  ...
  const semanticFailure = semanticResult.status === "rejected" ? semanticResult.reason : null
  const lexicalRowsRaw = lexicalResult.value
  const semanticRowsRaw = semanticResult.status === "fulfilled" ? semanticResult.value : []
  ...
  for (const row of lexicalRows) merged.set(itemId(row), { memory: memoryOf(row), memoryEmbedding: null, semanticScore: null })
  for (const row of semanticRows) {
    const id = itemId(row)
    const existing = merged.get(id)
    merged.set(id, { memory: existing?.memory || memoryOf(row), memoryEmbedding: row.memory_embedding ?? row.embedding ?? row.vector ?? null, semanticScore: row.semantic_similarity ?? null })
  }
  const candidates = [...merged.values()].map(({ memory, memoryEmbedding, semanticScore }) => {
    const candidate = buildHybridCandidate({
    ...
    if (semanticScore !== null && semanticScore !== undefined) {
      if (!Number.isFinite(Number(semanticScore)) || Number(semanticScore) < 0 || Number(semanticScore) > 1) throw new Error("SEMANTIC_SIMILARITY_INVALID")
      candidate.semantic_score = round(Number(semanticScore))
      candidate.semantic_available = true
      candidate.semantic_reason_codes = ["SEMANTIC_DB_SIMILARITY_AVAILABLE"]
    }
    return candidate
  ...
  return {
    ...
    degradation_mode: channelMode === "semantic_only" ? "SEMANTIC_ONLY"
      : semanticFailure ? "LEXICAL_ONLY_SEMANTIC_FAILED"
      : semanticEnabled && semanticRows.length > 0 ? "HYBRID_AVAILABLE" : "LEXICAL_ONLY",
    semantic_error_code: semanticFailure
      ? String(semanticFailure?.code || semanticFailure?.message || "SEMANTIC_FAILED").split(":")[0].slice(0, 80)
      : null,
  }
}
...
export async function retrieveXiaoCMemoriesOffline({
  ...
  const generated = await generateBoundedCandidates({ repository, userId, query: queryPlan.retrieval_query, lexicalTerms: queryPlan.lexical_terms, queryEmbedding, embeddingIdentity, channelMode, policy })
  const candidateIds = generated.candidates.map((item) => item.memory_id)
  const relationRows = relations ?? (repository.listRelations && candidateIds.length ? await repository.listRelations({ userId, memoryIds: candidateIds }) : [])
  ...
  const compatible = resolved.eligible_candidates
  const ranked = applySemanticDistributionAdmission(
    compatible.map((candidate) => rankEligibleCandidate(candidate, { retrievalTime, mode, explicitRecall: queryPlan.explicit_recall }, policy)).sort(rankingOrder),
    policy,
  )
  const passing = ranked.filter((item) => item.passes_threshold)
  const selectionPool = dedupeRanked(passing)
  const results = selectionPool.slice(0, requestedTopK)
  return {
    results,
