...
export async function generateBoundedCandidates({ repository, userId, query, queryEmbedding = null, embeddingIdentity = null, policy = XIAOC_MEMORY_RETRIEVAL_POLICY }) {
  if (!repository?.listLexicalCandidates) throw new Error("BOUNDED_LEXICAL_REPOSITORY_REQUIRED")
  const lexicalPromise = repository.listLexicalCandidates({ userId, query, limit: policy.lexicalCandidateLimit })
  const semanticEnabled = queryEmbedding !== null && typeof repository.listSemanticCandidates === "function"
  ...
  return {
    ...
    degradation_mode: semanticEnabled && semanticRows.length > 0 ? "HYBRID_AVAILABLE" : "LEXICAL_ONLY",
  }
}
...
export async function retrieveXiaoCMemoriesOffline({
  ...
  if (!String(query || "").trim() || requestedTopK === 0) return {
    results: [],
    trace: { policy_version: policy.version, query: { present: Boolean(String(query || "").trim()), mode, explicit_recall: explicitRecall === true }, raw_candidate_count: 0, eligible_count: 0, suppressed_count: 0, ranked_count: 0, selected_count: 0, threshold_rejected_count: 0, degradation_mode: "NO_QUERY", reason_code_counts: {}, latency_ms: { total: round(performance.now() - started) } },
  }
  const generated = await generateBoundedCandidates({ repository, userId, query, queryEmbedding, embeddingIdentity, policy })
  ...
  const ranked = resolved.eligible_candidates.map((candidate) => rankEligibleCandidate(candidate, { retrievalTime, mode, explicitRecall }, policy)).sort(rankingOrder)
  ...
  return {
    ...
    trace: {
      ...
      query: { present: true, mode, explicit_recall: explicitRecall === true },
