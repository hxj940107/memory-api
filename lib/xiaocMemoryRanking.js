...
export async function retrieveXiaoCMemoriesOffline({
  ...
  if (!queryPlan.should_retrieve || requestedTopK === 0) return {
    results: [],
    trace: { policy_version: policy.version, query_plan: { version: queryPlan.version, should_retrieve: false, grounding_strength: queryPlan.grounding_strength, reason_codes: queryPlan.reason_codes }, discovered_candidate_count: 0, raw_candidate_count: 0, eligible_count: 0, eligibility_rejected_count: 0, suppressed_count: 0, ranked_count: 0, ranked_pool_count: 0, selected_count: 0, degradation_mode: "NO_QUERY", reason_code_counts: {}, latency_ms: { total: round(performance.now() - started) } },
  }
  ...
  const ranked = compatible
    .map((candidate) => rankEligibleCandidate(candidate, { retrievalTime, mode, explicitRecall: queryPlan.explicit_recall }, policy))
    .sort(rankingOrder)
  const selectionPool = dedupeRanked(ranked)
  ...
  return {
    ...
    trace: {
      ...
      discovered_candidate_count: generated.candidates.length,
      raw_candidate_count: generated.candidates.length,
      eligible_count: resolved.eligible_candidates.length,
      eligibility_rejected_count: resolved.suppressed_candidates.length,
      suppressed_count: resolved.suppressed_candidates.length,
      ranked_count: ranked.length,
      ranked_pool_count: selectionPool.length,
      ...
      ranking_decisions: ranked.map((item, index) => ({ memory_id: item.candidate.memory_id, component_scores: item.component_scores, hybrid_score: item.hybrid_score, rank: index + 1, reason_codes: item.selection_reason_codes })),
