...
export async function retrieveXiaoCMemoriesOffline({
  ...
  const compatible = resolved.eligible_candidates.filter((candidate) => {
    ...
    if (!admitted) compatibilityRejected.push({
      memory_id: candidate.memory_id,
      signal_mode: "semantic_only",
      lexical_score: candidate.lexical_score > 0 ? round(candidate.lexical_score) : null,
      semantic_score: round(candidate.semantic_score),
      semantic_grounded_floor: policy.semanticAdmission.groundedMinimum,
      reason_codes: [
