...
const CURRENT_TIME_CLAIM_PATTERN = /(刚刚|这会儿|此刻|现在才|刚结束|刚回到|今晚正在|待会儿|等会儿|一会儿|明天)/
...
export function normalizeMomentCandidateForPublish(candidate, publishTime = new Date()) {
  ...
  const materialIsHistorical = localDateKey(eventTime) !== localDateKey(target) ||
    ageMs > 3 * 60 * 60 * 1000
  const hasCurrentTimeClaim = CURRENT_TIME_CLAIM_PATTERN.test(String(candidate?.text || ""))
  const requiresDelayedVoice = materialIsHistorical && hasCurrentTimeClaim
