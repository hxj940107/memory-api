...
export function parseActiveContextJudgeOutput(raw, { finishReason = null } = {}) {
  ...
  const proposalSection = parseNamedSection(text, "proactive_event_proposal", topLevel.value)
  const activeContext = normalizeActiveConversationContext(activeSection.value)
  const proposal = validateProposal(proposalSection.value)
  const activeErrorCode = activeContext
    ? null
    : activeSection.errorCode || "active_context_invalid_shape"
  const proposalErrorCode = proposalSection.errorCode || proposal.errorCode
  ...
  return {
    activeContext,
    proactiveEventProposal: proposal.value,
    diagnostics: {
