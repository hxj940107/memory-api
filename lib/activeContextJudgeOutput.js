...
function expandCompactProposal(value, { sourceMessageId = null, existingCandidatesById = new Map() } = {}) {
  ...
  return {
    ...
    matched_event_id: value.id ?? null,
    description: value.d || existingCandidatesById.get(value.id)?.description,
...
function expandCompactActiveContext(value) {
  ...
  return {
    items: (Array.isArray(value.i) ? value.i : []).map(item => ({
      ...
      source_message_id: item.src,
      last_referenced_message_id: item.ref,
...
export function parseActiveContextJudgeOutput(raw, {
  ...
  return {
    ...
    diagnostics: {
      ...
      output_format: compactOutput ? "compact_v1" : "verbose_legacy",
