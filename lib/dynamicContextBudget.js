...
export function buildDeterministicHistoryEpoch(messages, {
  ...
  const completedTurns = Number(completedUserTurns)
  if (!Number.isInteger(completedTurns) || completedTurns < 0 || !Number.isInteger(epochUserTurns) || epochUserTurns < 1) {
    return {
      ...baseline,
  ...
  const epochPosition = completedTurns % epochUserTurns
  ...
  const tailTokens = tailMessages.reduce(
    (total, item) => total + estimateTextTokens(item?.content) + 4,
    0
  )
  const combinedTokens = seedSelection.estimatedTokens + tailTokens
  const epochMessages = [...seedSelection.messages, ...tailMessages]
  const epochIds = new Set(epochMessages.map(item => String(item.id)))
  const preservesBaseline = baseline.messages.every(item => epochIds.has(String(item.id)))

  if (combinedTokens > tokenBudget + tailTokenAllowance || !preservesBaseline) {
    return {
      ...baseline,
      baselineMessages: baseline.messages,
      seedMessages: baseline.messages,
