...
export function normalizeMessageVoiceAsset(metadata) {
  ...
  return {
    ...
    usage_characters: voice.usage_characters !== null &&
      voice.usage_characters !== undefined &&
      Number.isFinite(Number(voice.usage_characters))
      ? Math.max(0, Number(voice.usage_characters))
      : null,
    estimated_cost_cny: voice.estimated_cost_cny !== null &&
      voice.estimated_cost_cny !== undefined &&
      Number.isFinite(Number(voice.estimated_cost_cny))
      ? Math.max(0, Number(voice.estimated_cost_cny))
      : null,
    price_cny_per_10k: voice.price_cny_per_10k !== null &&
      voice.price_cny_per_10k !== undefined &&
      Number.isFinite(Number(voice.price_cny_per_10k))
      ? Math.max(0, Number(voice.price_cny_per_10k))
      : null,
...
export async function signMessageVoicePlayback({
  ...
  if (
    !asset ||
    asset.content_hash !== buildMessageVoiceContentHash(message.content) ||
    (expectedVoiceVersion && asset.voice_version !== expectedVoiceVersion)
  ) {
...
export async function saveMessageVoice({
  ...
  const voice = {
    ...
    usage_characters: synthesis.usage_characters !== null &&
      synthesis.usage_characters !== undefined &&
      Number.isFinite(Number(synthesis.usage_characters))
      ? Math.max(0, Number(synthesis.usage_characters))
      : null,
    estimated_cost_cny: synthesis.estimated_cost_cny !== null &&
      synthesis.estimated_cost_cny !== undefined &&
      Number.isFinite(Number(synthesis.estimated_cost_cny))
      ? Math.max(0, Number(synthesis.estimated_cost_cny))
      : null,
    price_cny_per_10k: synthesis.price_cny_per_10k !== null &&
      synthesis.price_cny_per_10k !== undefined &&
      Number.isFinite(Number(synthesis.price_cny_per_10k))
      ? Math.max(0, Number(synthesis.price_cny_per_10k))
      : null,
    presentation: presentation === "voice_reply" ? "voice_reply" : null,
...
export async function prepareMessageVoicePlayback({
  ...
  try {
    const signed = await signMessageVoicePlayback({
      supabase, user_id, conversation_id, message_id, expectedVoiceVersion, expiresIn,
    })
    if (presentation === "voice_reply" && signed.voice.presentation !== "voice_reply") {
      const message = await loadAssistantMessage({ supabase, user_id, conversation_id, message_id })
      const voice = { ...signed.voice, presentation: "voice_reply" }
      const { error: updateError } = await supabase
        .from("messages")
        .update({ metadata: { ...(message.metadata || {}), voice } })
        .eq("id", message_id)
        .eq("user_id", user_id)
        .eq("conversation_id", conversation_id)
        .eq("role", "assistant")
  ...
  await saveMessageVoice({
    supabase, user_id, conversation_id, message_id, synthesis, presentation,
  })
