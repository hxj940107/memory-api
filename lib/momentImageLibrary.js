...
const OBVIOUS_IMAGE_RAIN_PATTERN = /雨|下雨|雨后|雨天|淋湿|积水|湿漉/
const IMAGE_FACT_RULES = [
  {
    image: /猫|橘猫|狗|狗狗|小狗|宠物|小动物/,
    source: /猫|橘猫|狗|狗狗|小狗|博美|宠物|小动物/,
    allowEntityGrounding: true,
  },
  {
    image: /咖啡|拿铁|蛋糕|甜品|可颂|早餐|汉堡|炸鸡/,
    source: /咖啡|拿铁|蛋糕|甜品|可颂|早餐|汉堡|炸鸡|吃|餐/,
  },
  {
    image: /驾驶|驾驶位|车内|方向盘|红灯/,
    source: /驾驶|开车|车里|车内|方向盘|红灯|通勤/,
  },
]
...
function hasEntityMention(sourceText, relations = []) {
  const source = normalizeEntityLabel(sourceText)
  if (!source) return false

  return relations.some(value => {
    const relation = normalizeEntityLabel(value)
    if (!relation) return false
    const characters = [...relation]
    const containsCjk = /[\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}\p{Script=Hangul}]/u.test(relation)
    if (characters.length < (containsCjk ? 2 : 3)) return false
    if (containsCjk) return source.includes(relation)

    const escaped = relation
      .replace(/[.*+?^${}()|[\]\\]/g, "\\$&")
      .replace(/\s+/g, "\\s+")
  ...
  const compatiblePeriods = period === "earlyMorning"
    ...
    : period === "morning"
      ...
      : period === "afternoon"
        ? ["afternoon", "daytime"]
        : [period]
  const timePeriods = Array.isArray(image.timePeriods) ? image.timePeriods : []
  const hasGroundedEntity = hasEntityMention(source, image.relations)
  const matchesPeriod = timePeriods.length === 0 ||
    compatiblePeriods.some(item => timePeriods.includes(item))
  const hasUnsupportedSubject = IMAGE_FACT_RULES.some(rule =>
    rule.image.test(imageSemantics)
    && !rule.source.test(source)
    && !(rule.allowEntityGrounding && hasGroundedEntity)
  )
  const hasRainFact = image.weather === "rain" || OBVIOUS_IMAGE_RAIN_PATTERN.test(imageSemantics)
  const hasSnowFact = image.weather === "snow" || SNOW_PATTERN.test(imageSemantics)
  const hasUnsupportedWeather = (hasRainFact && !RAIN_PATTERN.test(source)) ||
    (hasSnowFact && !SNOW_PATTERN.test(source))

  return matchesPeriod && !hasUnsupportedSubject && !hasUnsupportedWeather
}
