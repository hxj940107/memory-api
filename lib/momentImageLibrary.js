...
function getMomentTextPeriod(text, fallbackHour) {
  ...
  if (/早上|早晨|清晨|上午|早餐/.test(value)) return "morning"
  if (/中午|下午|白天/.test(value)) return "daytime"
...
export function isMomentImageCompatible(
  ...
  const matchesPeriod = image.timePeriods.includes(period)
...
export function resolveMomentImage(imageId, baseUrl) {
  const image = MOMENT_IMAGE_LIBRARY.find(item => item.id === imageId)
  const normalizedBaseUrl = String(baseUrl || "").replace(/\/$/, "")

  if (!image || !normalizedBaseUrl) return null
