...
export const XIAOC_MEMORY_LEXICAL_MAX_TERMS = 12
export const XIAOC_MEMORY_LEXICAL_MAX_TERM_CODEPOINTS = 128
...
const TRAILING_SHELL = /(?:这件事)?(?:吗|嘛|呢|呀|啊)[？?。！!]*$/u
...
function extractTerms(value) {
  ...
  return unique([...ascii, ...cjk].filter((term) =>
    !GENERIC_TOKENS.has(term) && [...term].length <= XIAOC_MEMORY_LEXICAL_MAX_TERM_CODEPOINTS
  ))
}

function boundedTerms(...groups) {
  return unique(groups.flat()).slice(0, XIAOC_MEMORY_LEXICAL_MAX_TERMS)
}
...
export function buildXiaoCMemoryQueryPlan(rawQuery, { explicitRecall = false, mode = "normal_current", grounding = null } = {}) {
  ...
  const lexicalTerms = boundedTerms(
    extractTerms(weakReferenceOnly ? "" : (substantivePayload || payload)),
    grounded.anchors.flatMap(extractTerms),
  )
  const shouldRetrieve = lexicalTerms.length > 0 && !(weakReferenceOnly && grounded.strength !== "STRONG")
