...
export class XiaoCMemoryDbRetrievalRepository {
  ...
  async listLexicalCandidates({ userId, lexicalTerms, limit }) {
    ...
    if (!terms.length || terms.length > XIAOC_MEMORY_LEXICAL_MAX_TERMS
      || terms.some((term) => [...term].length > XIAOC_MEMORY_LEXICAL_MAX_TERM_CODEPOINTS)) throw new Error("LEXICAL_TERMS_INVALID")
