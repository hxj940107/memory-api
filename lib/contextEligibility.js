...
function traceFor(candidate, reason, userRementionedNow, representationCompacted = false) {
  const eligibleForPrompt = reason === CONTEXT_ELIGIBILITY_REASONS.INJECTED
  ...
  return {
    ...
    retrieved: true,
    relevant: reason !== CONTEXT_ELIGIBILITY_REASONS.LOW_RELEVANCE,
    eligible_for_prompt: eligibleForPrompt,
    eligible_for_proactive_attention: false,
    injected: eligibleForPrompt,
    suppressed: !eligibleForPrompt,
    suppression_reason: reason,
    user_rementioned_now: userRementionedNow,
    representation_compacted: representationCompacted,
  }
}
...
export function evaluateContextCandidates(candidates, context = {}, options = {}) {
  ...
  const maxChars = Number.isFinite(Number(options.maxChars))
    ...
    : Infinity
  const maxInjected = Number.isInteger(options.maxInjected)
    ? Math.max(0, options.maxInjected)
    : Infinity
  let usedChars = 0

  for (const [index, rawCandidate] of (candidates || []).entries()) {
    if (injected.length >= maxInjected) break
    let candidate = normalizeCandidate(rawCandidate, index)
    if (!candidate.content) continue
    const originalContent = candidate.content
    let representationCompacted = false
    ...
    if (!reason && usedChars + candidate.content.length > maxChars) {
      const compacted = typeof options.compactForBudget === "function"
        ? options.compactForBudget(candidate, Math.max(0, maxChars - usedChars))
        : null
      if (compacted?.content && compacted.content.length <= Math.max(0, maxChars - usedChars)) {
        candidate = { ...candidate, ...compacted }
        representationCompacted = candidate.content !== originalContent
      } else {
    ...
    diagnostics.push(traceFor(candidate, reason, userRementionedNow, representationCompacted))
    seen.push(originalContent)
