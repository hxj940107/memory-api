...
function validateStructuredUserUpdate({ proposal, sourceMessage, state }) {
  const userUpdate = proposal?.user_update
  ...
  if (!userUpdate || typeof userUpdate !== "object") {
    return rejected("missing_user_update_evidence")
  }
  if (!USER_UPDATE_KINDS.has(userUpdate.kind)) {
    return rejected("invalid_user_update_kind")
  }
  if (!USER_UPDATE_EXPLICITNESS.has(userUpdate.explicitness)) {
    return rejected("invalid_user_update_explicitness")
  }
  if (!sourceEvidenceIsQuoted(sourceMessage?.content, userUpdate.evidence_text)) {
    return rejected("invalid_source_provenance")
  }
  if (
    userUpdate.time_evidence_text
    && !sourceEvidenceIsQuoted(sourceMessage?.content, userUpdate.time_evidence_text)
  ) {
    return rejected("invalid_time_evidence_provenance")
  }
  const requiredKind = state === "completed"
    ? "completed"
    : state === "cancelled" ? "cancelled" : null
  if (requiredKind && userUpdate.kind !== requiredKind) {
    return rejected("user_update_state_mismatch")
  }
  if (requiredKind && userUpdate.explicitness !== "explicit") {
    return rejected("unsupported_terminal_transition")
  }
  return {
    accepted: true,
    reason: "verified_current_user_update",
...
function validateContextualExistingUpdate({
  ...
  if (
    ...
    return rejected("contextual_assistant_not_adjacent")
  }
  ...
  if (!structuredUserUpdate?.accepted) return rejected(structuredUserUpdate?.reason || "contextual_update_missing_user_fact")
  ...
  if (
    ...
    && !windowChanged
    && !descriptionChanged
    && proposal.user_update?.explicitness === "none"
  ) {
  ...
  return {
    ...
    identitySource: isExplicitAssistantQuestion(contextualAssistantMessage.content)
      ? "assistant_question_existing_event"
      : "assistant_continuation_existing_event",
  }
}
...
export function applyProactiveEventProposal({
  ...
  const structuredUserUpdate = matched
    ? validateStructuredUserUpdate({ proposal, sourceMessage, state })
    : null
  ...
  if (matched && !structuredUserUpdate?.accepted) {
    return {
      candidates: current,
      diagnostics: diagnostic({
        event_id: matched.event_id,
        event_state: matched.state,
        merge_action: structuredUserUpdate?.reason || "missing_user_update_evidence",
        admission_reason: structuredUserUpdate?.reason || "missing_user_update_evidence",
        matched_event_id: matched.event_id,
        source_message_ids: matched.source_message_ids,
        last_user_update_message_id: matched.last_user_update.message_id,
        expected_window: matched.expected_window,
        ...
        fact_source: contextualEvidence.accepted
          ...
          : null,
        user_update: structuredUserUpdate?.userUpdate || proposal.user_update || null,
        evidence_verified: false,
      }),
    }
  }
  ...
  return {
    ...
    diagnostics: diagnostic({
      ...
      contextual_referent_verified: contextualEvidence.accepted,
      identity_source: contextualEvidence.accepted
        ? contextualEvidence.identitySource
        : null,
      fact_source: contextualEvidence.accepted
        ? "current_user_message"
        : null,
      user_update: proposal.user_update || null,
      evidence_verified: Boolean(structuredUserUpdate?.evidenceVerified),
    }),
  }
}
