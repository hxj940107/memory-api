...
export function privateAppInternalHeaders(env = process.env) {
  ...
  const headers = token ? { [PRIVATE_APP_TOKEN_HEADER]: token } : {}
  const previewBypass = String(env.PRIVATE_PREVIEW_VERCEL_PROTECTION_BYPASS_SECRET || "")
  if (env.VERCEL_ENV === "preview" && previewBypass) {
    headers[VERCEL_PROTECTION_BYPASS_HEADER] = previewBypass
  }
  return headers
}

export function authenticatedSelfCallHeaders(req, env = process.env) {
  const identity = req?.identity
  if (
    identity?.actorType !== "authenticated_user"
    || identity?.identitySource !== "verified_supabase_jwt"
  ) {
    return privateAppInternalHeaders(env)
  }
  ...
  if (!/^Bearer\s+\S+$/i.test(authorization)) {
    const error = new Error("verified_user_authorization_missing")
    error.code = "verified_user_authorization_missing"
    throw error
  }

  const compatibilityHeaders = privateAppInternalHeaders(env)
  delete compatibilityHeaders[PRIVATE_APP_TOKEN_HEADER]
