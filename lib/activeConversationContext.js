...
function itemIdentity(item) {
  return item?.source_message_id || item?.topic || null
}
...
export function normalizeActiveConversationContext(value, { includeSourceEvidence = false } = {}) {
  ...
  for (const rawItem of value.items) {
    ...
    items.push({
      ...(compactText(rawItem?.id, 120) ? { id: compactText(rawItem.id, 120) } : {}),
      topic,
      ...
      ...(compactText(rawItem?.source_role, 20)
        ? { source_role: compactText(rawItem.source_role, 20) }
        : {}),
      ...(compactText(rawItem?.evidence_text, 160)
        ? { evidence_text: compactText(rawItem.evidence_text, 160) }
        : {}),
      ...(rawItem?.provenance && typeof rawItem.provenance === "object"
        ? { provenance: rawItem.provenance }
        : {}),
      ...(compactText(rawItem?.created_at, 60)
        ? { created_at: compactText(rawItem.created_at, 60) }
        : {}),
...
export function resolveActiveConversationContext(
  ...
  const previousKeys = new Set(
    (normalizedPrevious?.items || []).map(item => itemIdentity(item))
  )
  ...
  const candidateKeys = new Set(normalizedCandidate.items.map(item => itemIdentity(item)))
  ...
  const candidateItems = normalizedCandidate.items
    .filter(item => item.status !== "resolved")
    .filter(item => {
      const existing = previousKeys.has(itemIdentity(item))
    ...
    .map(item => {
      const previousItem = previousBySource.get(itemIdentity(item))
      ...
      return { ...previousItem, ...storedItem, missed_turns: missedTurns }
    })

  // The judge returns a best-effort snapshot, but omission is not lifecycle
  // evidence. Carry omitted items forward and let only transient items age out.
  // An explicit resolved item with the same stable identity blocks carry-forward.
  const carriedItems = (normalizedPrevious?.items || [])
    .filter(item => !candidateKeys.has(itemIdentity(item)))
    .filter(item => !resolvedKeys.has(itemIdentity(item)))
    .map(item => ({
      ...item,
      missed_turns: Math.min(99, (item.missed_turns || 0) + 1),
    }))

  const items = [...candidateItems, ...carriedItems]
    .filter(item => (
      item.kind !== "transient"
