...
const SPECULATIVE = /(?:可能|也许|或许|大概|估计|猜测|说不定|不确定|好像)/
...
export function validateXiaoCMemoryNativeCaptureInput({
  ...
  if (!judgeResult?.save || !NATIVE_CAPTURE_CATEGORIES.has(judgeResult.category)) {
  ...
  if (!FIRST_PERSON.test(evidenceText) || DEVELOPMENT_OR_TEST.test(evidenceText)
    || MUTABLE_OR_PLANNED.test(evidenceText) || SPECULATIVE.test(evidenceText)) {
      ...
      if (data && embeddingProvider) {
        try {
          await ensureActiveMemoryEmbedding({ client, provider: embeddingProvider, logger, memory: {
            id: data, user_id: context.trustedUserId,
            canonical_content: eligibility.input.p_canonical_content,
            content_hash: hashEmbeddingInput(eligibility.input.p_canonical_content),
          } })
