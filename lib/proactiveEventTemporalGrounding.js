...
export function normalizeProactiveEventWindow(proposal, {
  ...
  const start = localBoundaryToUtc(local.start)
  const end = localBoundaryToUtc(local.end)
  ...
  return {
    proposal: {
      ...
      time_grounding: {
        ...
        user_message_created_at_utc: isoOrNull(userMessageCreatedAt),
    ...
    errorCode: validOrder ? null : "event_proposal_invalid_window_order",
  }
}
