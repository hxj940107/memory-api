...
export function filterContextEntries(entries, suppressionTexts, currentMessage = "") {
  const kept = []
  for (const rawEntry of entries || []) {
    const entry = String(rawEntry || "").trim()
    if (!entry) continue
    if (kept.some(existing => isContextuallyDuplicate(existing, entry))) continue

    const currentRelevance = currentMessage
      && isContextuallyDuplicate(entry, currentMessage)
    const suppressed = !currentRelevance && (suppressionTexts || [])
      .some(text => isContextuallyDuplicate(entry, text))
    if (!suppressed) kept.push(entry)
  }
  return kept
}
