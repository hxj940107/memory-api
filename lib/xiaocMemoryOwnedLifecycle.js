...
function ownedMemoryDisplayCategory(row) {
  ...
  if (/关系|伴侣|老婆|老公|小C|小c|回应|互动|陪伴|称呼|安全感|信任|被理解|被接受/.test(content)) {
    return "我们之间"
  }
  if (/经历|一起|第一次|曾经|以前|那次|时刻|完成|收到|去过|纪念/.test(content)) {
    return "一起经历过"
  }
  if (/喜欢|不喜欢|偏好|习惯|口味|称呼|希望被|喜欢被/.test(content)) {
    return "小小偏好"
  }
  return "关于你"
}

function ownedMemoryDisplayTitle(category) {
  return {
    "关于你": "关于你的这一点",
    "我们之间": "我们之间的记忆",
    "一起经历过": "一起经历过的事",
...
export function toOwnedMemoryView(row) {
  ...
  return {
    id: row.id,
    title: ownedMemoryDisplayTitle(category),
    content,
    category,
    tags: [],
    domains: [],
