...
test("mobile voice input switches modes before the large hold area records and sends", () => {
  ...
  assert.match(chat, /IOSOutputFormat\.LINEARPCM/)
  assert.match(chat, /sampleRate: 16000/)
  assert.match(chat, /numberOfChannels: 1/)
  assert.match(chat, /useAudioRecorder\(USER_VOICE_RECORDING_OPTIONS\)/)
  assert.match(chat, /prepareToRecordAsync\(USER_VOICE_RECORDING_OPTIONS\)/)
  assert.match(chat, /Platform\.OS === "ios" \? "audio\/wav" : "audio\/mp4"/)
