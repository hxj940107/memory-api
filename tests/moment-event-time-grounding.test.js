...
test("a candidate crossing a day is flagged without rewriting its voice", () => {
  ...
  assert.equal(result.corrected, false)
  assert.equal(result.requiresDelayedVoice, true)
  assert.equal(result.candidate.shareMode, "immediate")
  assert.equal(result.candidate.text, "刚刚她说要开始 debug。")
...
test("automatic Moment generation stays low-frequency but publication limits do not erase material", () => {
  ...
  assert.match(source, /自动和手动模式都允许返回 shouldPost false/)
  assert.doesNotMatch(source, /text\s*=\s*`突然想起/)
})
