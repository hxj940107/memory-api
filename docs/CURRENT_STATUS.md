...
- Weather-aware companionship keeps its full Shadow diagnostics and now has a fail-closed limited-send path behind `WEATHER_LIVE_SEND_ENABLED=true`. A qualified Nanjing weather signal must pass calendar/life-rhythm judgment, quiet hours, cooldown, daily proactive limit, recent-user-activity and same-signal dedupe; weather is fetched again immediately before persistence. The model may still decline, no fixed fallback copy exists, and a weather send consumes the competing due inactivity contact. No API Function or schema was added.
